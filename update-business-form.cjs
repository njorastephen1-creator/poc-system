...
console.log('Updating Business Registration Form...\n');
...
const counties = [
    ...
    'Machakos', 'Makueni', 'Murang\'a', 'Nyeri', 'Kirinyaga', 'Nyandarua',
    'Nakuru', 'Narok', 'Kajiado', 'Kericho', 'Baringo', 'Trans-Nzoia',
    'Kitui', 'Embu'
].sort();
...
async function renderMyBusiness(container) {
    ...
    try {
        ...
        if (businesses.length === 0) {
                ...
                document.getElementById('biz-form').addEventListener('submit', async (e) => {
                    ...
                    try {
                        await businessService.createBusiness({ 
                            ...
                            county: document.getElementById('biz-county').value,
                            opening_days: document.getElementById('biz-days').value,
                            business_hours: document.getElementById('biz-hours').value,
                            status: 'active' 
                        });
            ...
            let html = businesses.map(b => '<div class="bg-white p-6 rounded-xl shadow-md border border-gray-200 mb-4 card-hover"><h3 class="text-xl font-bold text-gray-900">' + b.name + '</h3><p class="text-gray-600 mt-2">' + b.description + '</p><p class="text-sm text-gray-500 mt-2 flex items-center gap-1"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"></path></svg>' + (b.county || b.address) + '</p>' + (b.opening_days ? '<p class="text-sm text-gray-500 mt-1">Open: ' + b.opening_days + ' (' + b.business_hours + ')</p>' : '') + '<span class="inline-block mt-3 px-3 py-1 bg-green-100 text-green-800 rounded-full text-xs font-semibold">' + b.status + '</span></div>').join('');
